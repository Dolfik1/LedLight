...
typedef struct 
{
    uint32_t nextChange;                ///< время следующей смены режима, мс.
    uint32_t period;                    ///< период смены режимов, с.
    uint8_t current;                    ///< текущий режим
    uint8_t fading;                     ///< скорость затемнения (0, если оно не используется)
    bool isRandom;                      ///< случайная смена режима
    bool needToFade;                    ///< признак необходимости включения эффекта затемнения
} StripCycle;
